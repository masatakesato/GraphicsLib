#ifndef	WORLEY_NOISE_GPU_H
#define	WORLEY_NOISE_GPU_H
...
class WorleyNoiseGPU : public IShader
{	
public:

	WorleyNoiseGPU();
	WorleyNoiseGPU( const TCHAR* filepath, GLSL_VERSION version );
	~WorleyNoiseGPU();
	...
	void SetMode( int mode )		{ if(mode<=2) m_Mode = mode; }
...
private:
	...
	int					m_Mode;	// 出力距離計算の方法 (0: F1, 1: F2, 2:F2-F1 )
...
//// WorleyNoiseGPUオブジェクトを生成するファクトリクラス
//class WorleyNoiseGPUFactory : IRendererFactory
//{
//private:
//
//	IRenderer* CreateInstance(){ return new WorleyNoiseGPU( "WorleyNoise_main.glsl" ); }
//
//public:
//
//	WorleyNoiseGPUFactory(){};
//	//~WorleyNoiseGPUFactory(){};
//};
