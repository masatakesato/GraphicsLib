...
//static const float g_Eps	= 0.5f;
	...
	// Default Constructor
	GPUVoxelFluid2D::GPUVoxelFluid2D()
	{
	...
	void GPUVoxelFluid2D::InitShader()
	{
		GLuint	program_id;
		...
		// Create shader
		m_Pass[ FLUID_PASS_ADVECT ].Init( _T("../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_Advect.glsl"), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_JACOBI].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_Jacobi.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_SUBTRACT_GRADIENT].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_SubtractGradient.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_COMPUTE_DIVERGENCE].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_ComputeDivergence.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_APPLY_IMPULSE].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_ApplyImpulse.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_APPLY_BUOYANCY].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_ApplyBuoyancy.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_FILL].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_Fill.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_ADVECT_MACCORMACK].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_AdvectMacCormack.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_COMPUTE_CURL].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_ComputeCurl.glsl" ), GLSL_VERSION::GLSL_430 );
		...
		// Create shader
		m_Pass[FLUID_PASS_VORTICITY_CONFINEMENT].Init( _T( "../../../test/GPUVoxelFluid2D/shaders/GPUVoxelFluid_VorticityConfinement.glsl" ), GLSL_VERSION::GLSL_430 );
	...
	void GPUVoxelFluid2D::BindVoxelData( FluidVoxelData2D *pVoxelData )
	{
		...
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_VELOCITY, 0.0f );
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_VELOCITY_BACK, 0.0f );

		ClearBuffer( FluidVoxelData2D::TEX_FLUID_DENSITY, 0.0f );
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_DENDITY_BACK, 0.0f );

		ClearBuffer( FluidVoxelData2D::TEX_FLUID_PRESSURE, 0.0f );
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_PRESSURE_BACK, 0.0f );

		ClearBuffer( FluidVoxelData2D::TEX_FLUID_TEMPERATURE, m_AmbientTemperature );
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_TEMPERATURE_BACK, m_AmbientTemperature );

		ClearBuffer( FluidVoxelData2D::TEX_FLUID_DIVERGENCE, 0.0f );
		ClearBuffer( FluidVoxelData2D::TEX_FLUID_OBSTACLES, 0.0f );
	...
	void GPUVoxelFluid2D::UnbindVoxelData()
	{
		m_refFluidVoxelData	= NULL;
		InitZero( m_DispatchGroups );
	...
	void GPUVoxelFluid2D::AddObstacles( int x, int y, float radius )
	{
	...
	void GPUVoxelFluid2D::RemoveObstacles( int x, int y, float radius )
	{
		const Vec2f pos		= { float(x), float(y) };
	...
	void GPUVoxelFluid2D::AddSmoke( int x, int y, float dx, float dy, float radius )
	{
		const Vec2f pos	= { float(x), float(y) };
	...
	void GPUVoxelFluid2D::Advect( Texture2D* velocity, Texture2D* source, Texture2D* obstacles, Texture2D* dest, float dissipation, float timestep )
	{
		// Bind Inputs
	...
	void GPUVoxelFluid2D::Jacobi( Texture2D* x, Texture2D* b, Texture2D* obstacles, Texture2D* dest, float alpha, float invbeta )
	{
		// Bind Inputs
	...
	void GPUVoxelFluid2D::Jacobi( SwapTexture2D* x, Texture2D* b, Texture2D* obstacles, int numIter, float alpha, float invbeta )
	{
	...
	void GPUVoxelFluid2D::SubtractGradient( Texture2D* velocity, Texture2D* pressure, Texture2D* obstacles, Texture2D* dest )
	{
		// Bind Inputs
	...
	void GPUVoxelFluid2D::ComputeDivergence( Texture2D* velocity, Texture2D* obstacles, Texture2D* dest )
	{
		// Bind Inputs
		GL_SAFE_CALL( glBindImageTexture( 0, velocity->TexID(), 0, GL_FALSE, 0, GL_READ_ONLY, velocity->GLInternalFormat() ) );	// g_TexVelocity
	...
	void GPUVoxelFluid2D::ApplyImpulse( Texture2D* dest, const Vec2f& position, float value, float radius )
	{
		// Bind Inputs
	...
	void GPUVoxelFluid2D::ApplyBuoyancy( Texture2D* velocity, Texture2D* temperature, Texture2D* density, Texture2D* dest )
	{
		// Bind Inputs
		GL_SAFE_CALL( glBindImageTexture( 0, velocity->TexID(), 0, GL_FALSE, 0, GL_READ_ONLY, velocity->GLInternalFormat() ) );	// g_TexVelocity
	...
	void GPUVoxelFluid2D::Fill( Texture2D* dest, const Vec2f& position, const Vec3f& fillVal, float radius )
	{
		// Bind Inputs
	...
	void GPUVoxelFluid2D::ClearBuffer( int tex_channel, float val )
	{
...
//GLErrorCheck();
	...
	void GPUVoxelFluid2D::AdvectMacCormack( Texture2D* velocity, Texture2D* source, Texture2D* obstacles, Texture2D* dest, float dissipation, float timestep )
	{
	...
	void GPUVoxelFluid2D::ComputeCurl( Texture2D* velocity, Texture2D* dest, float timestep )
	{
		// Bind Inputs
		GL_SAFE_CALL( glBindImageTexture( 0, velocity->TexID(), 0, GL_FALSE, 0, GL_READ_ONLY, velocity->GLInternalFormat() ) );// g_TexVelocity
	...
	void GPUVoxelFluid2D::ApplyVorticityConfinement( Texture2D* velocity, Texture2D* curl, Texture2D* dest, float timestep )
	{
		// Bind Inputs
		GL_SAFE_CALL( glBindImageTexture( 0, velocity->TexID(), 0, GL_FALSE, 0, GL_READ_ONLY, velocity->GLInternalFormat() ) );// g_TexVelocity
